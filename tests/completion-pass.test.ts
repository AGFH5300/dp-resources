...
describe('deployment completion pass', () => {
  ...
  it('implements resumable indexing beyond 500 rows with persisted queue and stale cleanup after completion', () => {
    ...
    expect(drive).toContain('crawlDriveIndexChunk');
    expect(drive).toContain('maxItems = options.maxItems ?? 500');
  });
