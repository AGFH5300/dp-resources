...
describe('preview quality and interaction repair',()=>{
  it('removes student-visible technical preview copy',()=>{
    ...
    for (const text of ['Protected','Native Google Sheets preview','Read-only presentation outline','browser-local','Visual slide fidelity']) expect(source).not.toContain(text);
  });
  ...
  it('range parser validates single byte ranges and rejects bad requests',()=>{
    ...
    expect(parseSingleByteRange('bytes=100-', 1000)).toMatchObject({kind:'range', header:'bytes=100-999'});
    expect(parseSingleByteRange('bytes=-50', 1000)).toMatchObject({kind:'range', header:'bytes=950-999'});
    for (const r of ['bytes=0-1,3-4','items=0-1','bytes=1000-1001','bytes=20-10','bytes=-0']) expect(parseSingleByteRange(r, 1000)).toMatchObject({kind:'invalid', total:1000});
  ...
  it('pptx outline is removed while LibreOffice is unavailable',()=>{
    expect(existsSync('app/resource/[fileId]/presentation-outline.tsx')).toBe(false);
