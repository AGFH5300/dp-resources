...
import {
  byteRangeLength,
  expectedPdfContentRange,
  validatePdfRangeUpstream,
} from '../lib/pdf-range-integrity';
import {
...
describe('private PDF preview derivatives', () => {
  ...
  it('uses private provider metadata and resumable image plus text preparation', () => {
    const originalMigration = read(
      'supabase/migrations/20260714110000_private_pdf_preview_derivatives.sql',
    );
    const providerMigration = read(
      'supabase/migrations/20260714150000_pdf_preview_r2_storage.sql',
    );
    const searchMigration = read(
      'supabase/migrations/20260714193000_pdf_preview_search_text.sql',
    );
    const exactSearchMigration = read(
      'supabase/migrations/20260714220000_pdf_preview_exact_search_geometry.sql',
    );
    ...
    expect(originalMigration).toContain(
      "values ('pdf-previews', 'pdf-previews', false",
    );
    ...
    expect(worker.indexOf('if (!readyPages.has(1)) {')).toBeLessThan(
      worker.indexOf('for (let start = 2'),
    );
...
describe('PDF range route integrity', () => {
  it('normalizes beginning, middle, open-ended, and suffix ranges', () => {
    ...
    expect(suffix).toMatchObject({
      ...
      header: 'bytes=9500-9999',
    });
    expect(byteRangeLength(beginning)).toBe(1024);
    expect(expectedPdfContentRange(middle, 10_000)).toBe(
  ...
  it('rejects invalid and oversized requests', () => {
    for (const value of [
      ...
      expect(parseSingleByteRange(value, 10_000)).toMatchObject({
        kind: 'invalid',
        total: 10_000,
      });
    }
    ...
    expect(read('app/api/resource/[fileId]/pdf-content/route.ts')).toContain(
  ...
  it('requires exact upstream 206, Content-Range, and Content-Length semantics', () => {
    ...
    const good = new Response(new Uint8Array(100), {
      status: 206,
      headers: {
    ...
    const ignored = new Response(null, {
      status: 200,
      headers: { 'content-length': '1000' },
    });
    const wrongRange = new Response(null, {
      status: 206,
      headers: { 'content-range': 'bytes 0-99/1000', 'content-length': '100' },
    });
    const wrongLength = new Response(null, {
      status: 206,
      headers: {
        'content-range': 'bytes 100-199/1000',
        'content-length': '99',
    ...
    expect(validatePdfRangeUpstream(decision, 1000, wrongRange)).toBe(
      'content-range',
    );
    expect(validatePdfRangeUpstream(decision, 1000, wrongLength)).toBe(
      'content-length',
    );
  });
  ...
  it('keeps missing, wrong-file, and expired preview authorization fail-closed', () => {
    ...
    try {
      ...
      expect(
        verifyPdfPreviewSession(created.token, 'another-file', now),
      ).toBeNull();
      expect(
        verifyPdfPreviewSession(
          created.token,
      ...
      const verified = verifyPdfPreviewSession(
        created.token,
        'drive-file-1',
        now,
      );
      ...
      if (previousSecret === undefined)
        delete process.env.PDF_PREVIEW_SESSION_SECRET;
