...
describe('remaining library regressions repair', () => {
  it('bottom-right context menu is portaled, fixed, positioned before paint, and exposes every action', () => {
    ...
    for (const action of ['Open in new tab', 'Download', 'ShareButton', 'SaveButton', 'Details', 'ReportResourceDialog']) {
  ...
  it('desktop context menu clamps to the visual viewport and scrolls internally when too tall', () => {
    ...
    expect(s).toContain("window.visualViewport?.addEventListener('resize',place)");
    expect(s).toContain("window.visualViewport?.addEventListener('scroll',place)");
  });
  ...
  it('folder-size SQL aggregates batches safely, including more than 1,000 rows and special folder names', () => {
    const sql = read('supabase/migrations/20260707120000_folder_size_summaries_rpc.sql');
    expect(sql).toContain('create or replace function public.dp_folder_size_summaries(folder_ids text[])');
    expect(sql).toContain('join unnest(folder_ids)');
    expect(sql).toContain('sum(file.size_bytes) filter (where file.size_bytes is not null)');
    expect(sql).toContain('count(file.drive_file_id)');
    expect(sql).toContain('count(file.size_bytes)');
    expect(sql).toContain("left(file.path, length(folder.path) + 3) = folder.path || ' / '");
  ...
  it('folder-size RPC is narrowly executable by service_role only', () => {
    const sql = read('supabase/migrations/20260707120000_folder_size_summaries_rpc.sql');
    expect(sql).toContain('revoke all on function public.dp_folder_size_summaries(text[]) from public');
    expect(sql).toContain('revoke all on function public.dp_folder_size_summaries(text[]) from anon');
    expect(sql).toContain('revoke all on function public.dp_folder_size_summaries(text[]) from authenticated');
    expect(sql).toContain('grant execute on function public.dp_folder_size_summaries(text[]) to service_role');
  });

  it('visible-folder batching and no-estimate behavior are preserved', () => {
    expect(read('lib/indexed-folder-view.ts')).toContain("filter((r) => r.is_folder).map((r) => r.drive_file_id)");
