...
describe('resource usage production hardening', () => {
  ...
  it('renders admin analytics leaderboard and detail drill-downs only in Admin', () => {
    ...
    expect(console).toContain('href={`/resource/${r.file_id}`}')
    expect(console).toContain('View stats')
    expect(console).toContain('View user usage')
    expect(console).toContain('No usage data yet. Open a resource as an approved user and wait at least one heartbeat interval.')
