...
describe('disposable domain policy', () => {
  it('normalizes email and returns authoritative RPC decisions', async () => {
    const rpc = vi
      .fn()
      .mockResolvedValue({
        data: { allowed: false, domain: 'epaynine.com', reason: 'blocked' },
        error: null,
      });
    await expect(
      getEmailDomainPolicy({ rpc } as never, ' User@EPAYNINE.com '),
    ).resolves.toEqual({
      allowed: false,
      domain: 'epaynine.com',
      reason: 'blocked',
    });
    expect(rpc).toHaveBeenCalledWith('dp_resource_email_domain_policy', {
      p_email: 'user@epaynine.com',
    });
  });
  ...
  it('throws on RPC failure so routes fail closed', async () => {
    ...
    await expect(
      getEmailDomainPolicy({ rpc } as never, 'student@example.edu'),
    ).rejects.toThrow('Unable to validate email domain policy');
  });
});
...
describe('signup route behavior', () => {
  async function loadStartSignup(policy: { allowed: boolean } | Error) {
    ...
    const rpc = vi.fn().mockImplementation((name: string) => {
      if (name === 'dp_resource_username_availability_status')
        return Promise.resolve({ data: 'available', error: null });
      if (name === 'dp_resource_is_email_available')
    ...
    vi.doMock('../lib/rate-limit', () => ({
      privacySafeRequestKey: vi.fn(() => 'k'),
      rateLimit: vi.fn(async () => ({ ok: true })),
    }));
    vi.doMock('../lib/supabase-server', () => ({
      createClient: vi.fn(async () => ({ rpc, auth: { signInWithOtp } })),
    }));
    vi.doMock('../lib/disposable-email', async () => ({
      ...((await vi.importActual('../lib/disposable-email')) as object),
      getEmailDomainPolicy: vi.fn(async () => {
        if (policy instanceof Error) throw policy;
        return { domain: 'epaynine.com', reason: null, ...policy };
      }),
    }));
    return {
      route: await import('../app/api/auth/start-signup/route'),
      signInWithOtp,
    };
  }
  ...
  it('does not send OTP for blocked email', async () => {
    ...
    const res = await route.POST(
      req('https://app.test/api/auth/start-signup', {
        username: 'student1',
        fullName: 'Student One',
        email: 'user@epaynine.com',
      }),
    );
  ...
  it('sends OTP for allowed email', async () => {
    ...
    expect(signInWithOtp).toHaveBeenCalledWith(
      expect.objectContaining({ email: 'user@example.edu' }),
    );
  });

  it('fails closed when domain RPC fails', async () => {
    const { route, signInWithOtp } = await loadStartSignup(
      new Error('rpc down'),
    );
    const res = await route.POST(
      req('https://app.test/api/auth/start-signup', {
...
describe('availability route behavior', () => {
  it('returns invalid for blocked domains before email availability RPC', async () => {
    ...
    const res = await route.GET(
      new Request(
        'https://app.test/api/auth/availability?type=email&value=user@epaynine.com',
      ),
    );
    expect(res.status).toBe(400);
    expect(await res.json()).toMatchObject({
...
describe('suspension API behavior', () => {
  function tableMock(
    ...
    const update = vi.fn((values: unknown) => ({
      eq: vi.fn(async () => {
        calls.push(`membership:${JSON.stringify(values)}`);
        return { error: null };
      }),
    }));
    const insert = vi.fn(async (values: { action?: string }) => {
      calls.push(`audit:${values.action}`);
      return {
        error: opts.auditError
          ? { code: 'AUDIT', message: 'audit failed' }
          : null,
      };
    });
    const upsert = vi.fn(async (values: unknown) => {
      calls.push(`domain:${JSON.stringify(values)}`);
      return {
        error: opts.domainError
          ? { code: 'DOMAIN', message: 'domain failed' }
          : null,
      };
    });
    const from = vi.fn((name: string) => {
      if (name === 'dp_resource_memberships')
        return {
          select: vi.fn(() => ({
            eq: vi.fn(() => ({
    ...
    const updateUserById = vi.fn(
      async (_id: string, payload: { ban_duration: string }) => {
        calls.push(`auth:${payload.ban_duration}`);
        return {
          error:
            payload.ban_duration === 'none' && opts.unbanError
              ? { code: 'UNBAN', message: 'no' }
              : payload.ban_duration !== 'none' && opts.banError
                ? { code: 'BAN', message: 'no' }
                : null,
        };
      },
    ...
    const rpc = vi.fn(async () => ({
      data: opts.policy ?? { allowed: true, matched_domain: null },
      error: null,
    }));
    return {
      client: { from, rpc, auth: { admin: { updateUserById } } },
      calls,
      updateUserById,
      upsert,
      rpc,
  ...
  async function loadSuspension(m: ReturnType<typeof tableMock>) {
    ...
    vi.doMock('../lib/auth', () => ({
      requireAdmin: vi.fn(async () => ({
        user: { id: adminId },
        membership: { email: 'admin@example.edu' },
      })),
    }));
    vi.doMock('../lib/supabase-admin', () => ({
      createSupabaseAdminClient: vi.fn(() => m.client),
    }));
  ...
  it('updates membership before Auth ban, audits suspend and block_domain, and never deletes users', async () => {
    ...
    const res = await route.PATCH(
      new Request('https://app.test/api/admin/users/x/suspension', {
        method: 'PATCH',
        body: JSON.stringify({
          suspended: true,
          reason: 'Abuse report',
  ...
  it('returns audit warning while preserving successful suspension', async () => {
    ...
    const res = await route.PATCH(
      new Request('https://app.test/api/admin/users/x/suspension', {
        method: 'PATCH',
        body: JSON.stringify({ suspended: true, reason: 'Abuse report' }),
      }),
      { params: Promise.resolve({ id: userId }) },
    );
    expect(res.status).toBe(200);
    expect(await res.json()).toMatchObject({
      ok: true,
      suspended: true,
      warnings: [expect.stringContaining('audit log')],
    });
  });
  ...
  it('rejects suspending admins or the current admin', async () => {
    for (const target of [
      { id: adminId, email: 'me@example.edu', role: 'user' },
      { id: userId, email: 'admin@example.edu', role: 'admin' },
    ]) {
      vi.resetModules();
      ...
      const res = await route.PATCH(
        ...
        { params: Promise.resolve({ id: target.id as string }) },
      );
  ...
  it('does not overwrite an exact explicit allow rule but still suspends the user', async () => {
    const m = tableMock({
      target: { id: userId, email: 'student@diaestudents.com', role: 'user' },
      policy: { allowed: true, matched_domain: 'diaestudents.com' },
    });
    const route = await loadSuspension(m);
    const res = await route.PATCH(
      new Request('https://app.test/api/admin/users/x/suspension', {
    ...
    expect(await res.json()).toMatchObject({
      ok: true,
      suspended: true,
      warnings: [expect.stringContaining('explicit allow rule')],
    });
  ...
  it('protects a subdomain when a parent-domain allow rule is most-specific', async () => {
    const m = tableMock({
      target: { id: userId, email: 'student@mail.school.edu', role: 'user' },
      policy: { allowed: true, matched_domain: 'school.edu' },
    });
    const route = await loadSuspension(m);
    const res = await route.PATCH(
      new Request('https://app.test/api/admin/users/x/suspension', {
  ...
  it('does not resubmit an already-blocked domain', async () => {
    const m = tableMock({
      target: { id: userId, email: 'bad@throwaway.test', role: 'user' },
      policy: { allowed: false, matched_domain: 'throwaway.test' },
    });
    const route = await loadSuspension(m);
    const res = await route.PATCH(
      new Request('https://app.test/api/admin/users/x/suspension', {
    ...
    expect(res.status).toBe(200);
    expect(await res.json()).toMatchObject({
      warnings: [expect.stringContaining('already blocked')],
    });
    expect(m.upsert).not.toHaveBeenCalled();
  });
});
...
describe('migration coverage', () => {
  const migration = readFileSync(
    'supabase/migrations/20260710153000_disposable_email_and_user_suspension.sql',
    'utf8',
  );
  it('uses the existing membership trigger function and atomic corrected hook permissions', () => {
    ...
    expect(migration).not.toContain(
      'dp_resource_create_membership_for_new_user',
    );
    expect(migration).not.toMatch(/create\s+trigger/i);
    expect(migration).toContain("return '{}'::jsonb");
    expect(migration).toMatch(
      /create or replace function public\.dp_before_user_created[\s\S]*?language plpgsql\nset search_path = ''/,
    );
    expect(migration).toContain(
  ...
  it('returns only a generic reason from the public domain policy for blocked rules', () => {
    const policyFunction =
      migration.match(
        /create or replace function public\.dp_resource_email_domain_policy[\s\S]*?\n\$\$;/,
      )?.[0] ?? '';
    ...
    expect(policyFunction).not.toContain(
      'Internal abuse investigation details',
    );
  });
