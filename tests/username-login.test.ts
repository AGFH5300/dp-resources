...
const profileClient = readFileSync('lib/account-profile-client.ts', 'utf8');
...
describe('username account display', () => {
  it('loads the signed-in profile username and avatar through the client-safe header', () => {
    ...
    expect(appHeader).toContain('loadAccountProfile');
    expect(appHeader).toContain('peekCachedAccountProfile');
    expect(profileClient).toContain("fetch('/api/account/profile'");
    expect(appHeader).not.toContain('@supabase/');
    expect(profileClient).not.toContain('@supabase/');
