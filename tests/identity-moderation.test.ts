...
describe('identity moderation policy', () => {
  ...
  it('rejects inappropriate full name and email local-part only', () => {
    ...
    expect(validateEmailLocalPartIdentity('n.a.z.i@example.edu').ok).toBe(
      false,
    );
    expect(validateEmailLocalPartIdentity('student@sexeducation.edu').ok).toBe(
      true,
    );
  });
...
describe('signup enforcement wiring', () => {
  ...
  it('blocks before OTP is sent and availability is neutral', () => {
    expect(signup.indexOf('validateUsernameIdentity')).toBeLessThan(
      signup.indexOf('signInWithOtp'),
    );
  ...
  it('repairs database username validation for literal repeated underscores only', () => {
    expect(currentSchema).toContain(
      'create or replace function public.dp_identity_validate_username',
    );
    expect(usernameRepairMigration).toContain(
      'create or replace function public.dp_identity_validate_username',
    );
    expect(currentSchema).toContain("position('__' in p_username) > 0");
    expect(usernameRepairMigration).toContain(
      "position('__' in p_username) > 0",
    );
