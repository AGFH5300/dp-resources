...
describe('formatMimeType', () => {
  it('formats common document MIME types with friendly labels', () => {
    ...
    expect(formatMimeType('application/vnd.ms-excel')).toBe(
      'Excel spreadsheet',
    );
    expect(
      formatMimeType(
  ...
  it('formats Google Workspace MIME types with friendly labels', () => {
    ...
    expect(formatMimeType('application/vnd.google-apps.document')).toBe(
      'Google Doc',
    );
    expect(formatMimeType('application/vnd.google-apps.presentation')).toBe(
      'Google Slides',
    );
  });
