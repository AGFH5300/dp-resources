...
describe('signup availability RPC exposure', () => {
  it('runs database availability checks with the server service-role client', () => {
    const availability = read('app/api/auth/availability/route.ts');
    const signup = read('app/api/auth/start-signup/route.ts');

    expect(availability).toContain("import { createSupabaseAdminClient } from '@/lib/supabase-admin'");
    expect(availability).toContain('const supabase = createSupabaseAdminClient()');
    expect(signup).toContain('const admin = createSupabaseAdminClient()');
    expect(signup).toContain("admin.rpc('dp_resource_username_availability_status'");
