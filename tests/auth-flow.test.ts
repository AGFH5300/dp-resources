...
const verifyOtpForm = readFileSync(
  'app/auth/verify-otp/verify-otp-form.tsx',
  'utf8',
);
const inboxShortcuts = readFileSync('components/inbox-shortcuts.tsx', 'utf8');
...
describe('auth redirects and public entry points', () => {
  it('keeps the public homepage indexable while redirecting legacy auth entry points', () => {
    ...
    expect(authPage).toContain(
      "params.mode === 'signup' ? '/auth/sign-up' : '/auth/login'",
    );
  });
  ...
  it('uses free/open public wording instead of gated marketing language', () => {
    ...
    expect(publicCopy).not.toMatch(
      /private|restricted|protected|request access/i,
    );
  });
...
describe('safe next-path handling', () => {
  it('accepts only safe internal return paths', () => {
    expect(safeInternalReturnPath('/library?folder=abc#top', '/library')).toBe(
      '/library?folder=abc#top',
    );
    expect(safeInternalReturnPath('https://evil.example', '/library')).toBe(
      '/library',
    );
    expect(safeInternalReturnPath('//evil.example/path', '/library')).toBe(
      '/library',
    );
    expect(safeInternalReturnPath(String.raw`/\evil`, '/library')).toBe(
      '/library',
    );
    expect(safeInternalReturnPath('%252F%252Fevil.example', '/library')).toBe(
      '/library',
    );
  });
});
...
describe('OTP request and verification flow', () => {
  ...
  it('shows clean inbox shortcuts and spam guidance on the OTP screen using local assets', () => {
    ...
    expect(verifyOtpForm).toContain('InboxShortcuts');
    expect(verifyOtpForm).toContain('check your spam or junk folder');
    expect(inboxShortcuts).toContain(
      'https://mail.google.com/mail/u/0/#inbox',
    );
    expect(inboxShortcuts).toContain('https://outlook.live.com/mail/0/inbox');
    expect(inboxShortcuts).toContain('mailto:');
    expect(inboxShortcuts).toContain('/brand/gmail-icon.svg');
    expect(inboxShortcuts).toContain('/brand/outlook-icon.svg');
    ...
    expect(inboxShortcuts).not.toContain('gmail_2020q4_48dp.png');
    expect(inboxShortcuts).not.toContain('outlook_48x1.svg');
    expect(inboxShortcuts).not.toContain('res-1.cdn.office.net');
    expect(inboxShortcuts).not.toContain('gstatic.com');
  });
});
...
const awaitingApprovalPage = readFileSync(
...
describe('compatibility redirect and OTP docs', () => {
  ...
  it('uses a migration timestamp later than the deployed 20260701102923 migration', () => {
    ...
    expect(
      existsSync(
        `supabase/migrations/${migrationTimestamp}_dp_resource_profiles_auth.sql`,
      ),
    ).toBe(true);
    expect(
      existsSync(
        'supabase/migrations/20260701000000_dp_resource_profiles_auth.sql',
      ),
    ).toBe(false);
