...
const page = (words: PdfSearchGeometryPayload['w']): PdfSearchGeometryPayload => ({
  v: 1,
  p: 12,
  w: words,
});
...
describe('PDF exact search geometry', () => {
  ...
  it('returns every repeated occurrence of a word on the page', () => {
    const matches = findPdfSearchMatches(page([
      ['Triangle', 0.10, 0.10, 0.08, 0.025, 0],
      ['and', 0.20, 0.10, 0.04, 0.025, 0],
      ['triangle', 0.30, 0.10, 0.08, 0.025, 0],
    ]), 'triangle');
  ...
  it('merges a phrase on one line into one highlight rectangle', () => {
    const matches = findPdfSearchMatches(page([
      ...
      ['triangle', 0.17, 0.20, 0.09, 0.025, 3],
    ]), 'right triangle');
  ...
  it('uses separate rectangles when a phrase crosses a line boundary', () => {
    const matches = findPdfSearchMatches(page([
      ['right', 0.82, 0.20, 0.07, 0.025, 7],
      ['triangle', 0.10, 0.25, 0.09, 0.025, 8],
    ]), 'right triangle');
  ...
  it('matches case and punctuation the way a user expects', () => {
    expect(normalizePdfSearchText('  RIGHT—Triangle!  ')).toBe('right triangle');
    const matches = findPdfSearchMatches(page([
      ['Right,', 0.10, 0.30, 0.06, 0.025, 9],
      ['TRIANGLE!', 0.17, 0.30, 0.10, 0.025, 9],
    ]), 'right triangle');
    expect(matches).toHaveLength(1);
  });
});
