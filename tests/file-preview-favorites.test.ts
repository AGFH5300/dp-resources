...
describe('file preview compatibility and favourite reliability',()=>{
  ...
  it('content/open routes forward safe Range requests and return 206 headers without weakening auth',()=>{
    for (const f of ['app/api/resource/[fileId]/content/route.ts','app/api/files/[fileId]/open/route.ts']) { const s=read(f); expect(s.indexOf('requireMember()')).toBeLessThan(s.indexOf('req.headers.get(\'range\')')); expect(s).toContain('needsRangeSupport(meta.mimeType, meta.name)'); expect(s).toContain('parseSingleByteRange(requestedRange, meta.size)'); expect(s).toContain("headers.set('accept-ranges', 'bytes')"); expect(s).toContain("headers.set('content-range'"); expect(s).toContain('status: shouldServeRange && contentRange ? 206 : 200'); expect(s).toContain('ifRangeMatches') }
  });
  it('maps every currently indexed MIME type to a non-generic preview path',()=>{
    ...
    for (const [mime,name] of cases) { const cap=getResourceCapability(mime,name,false); expect(cap.previewMode).not.toBe('pptx-outline'); if(name !== 'x.pptx') expect(cap.generic).toBe(false); }
  ...
  it('has secure local XLSX preview and clean PPTX/download fallback',()=>{
    expect(read('app/resource/[fileId]/xlsx-preview.tsx')).toContain("import('jszip')");
    const s=read('app/resource/[fileId]/resource-preview.tsx');
    expect(s).toContain('Preview unavailable');
    expect(s).not.toContain('Read-only presentation outline');
    expect(s).not.toContain('presentation-outline');
