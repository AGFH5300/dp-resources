...
describe('file preview compatibility and favourite reliability', () => {
  it('uses server-batched favourite state and a shared client store, avoiding SaveButton mount fetch flash', () => {
    ...
    expect(read('components/resource-actions.tsx')).toContain(
      'aria-pressed={saved}',
    );
  });
  ...
  it('content/open routes forward safe Range requests and return 206 headers without weakening auth', () => {
    for (const f of [
      ...
      const s = read(f);
      expect(s.indexOf('requireMember()')).toBeLessThan(
        s.indexOf("req.headers.get('range')"),
      );
      expect(s).toContain('needsRangeSupport(meta.mimeType, meta.name)');
      expect(s).toContain('parseSingleByteRange(requestedRange, meta.size)');
      expect(s).toContain("headers.set('accept-ranges', 'bytes')");
      expect(s).toContain("headers.set('content-range'");
      expect(s).toContain(
        'status: shouldServeRange && contentRange ? 206 : 200',
      );
      expect(s).toContain('ifRangeMatches');
    }
  });
  it('maps every currently indexed MIME type to a non-generic preview path', () => {
    ...
    for (const [mime, name] of cases) {
      const cap = getResourceCapability(mime, name, false);
      expect(cap.previewMode).not.toBe('pptx-outline');
      if (name !== 'x.pptx') expect(cap.generic).toBe(false);
    }
    expect(
      getResourceCapability(
        'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
  ...
  it('has secure local XLSX preview and clean PPTX/download fallback', () => {
    expect(read('app/resource/[fileId]/xlsx-preview.tsx')).toContain(
      "import('jszip')",
    );
    const s = read('app/resource/[fileId]/resource-preview.tsx');
