...
describe('bootstrap admin approval timestamp handling', () => {
  it('preserves an existing approved_at timestamp when promoting an admin', () => {
    expect(bootstrapAdminMembershipUpdate({ approved_at: '2026-01-02T03:04:05.000Z' }, '2026-07-01T00:00:00.000Z')).toEqual({
      role: 'admin',
      is_approved: true,
  ...
  it('sets approved_at when the membership has no approval timestamp', () => {
    expect(bootstrapAdminMembershipUpdate({ approved_at: null }, '2026-07-01T00:00:00.000Z')).toEqual({
      role: 'admin',
      is_approved: true,
