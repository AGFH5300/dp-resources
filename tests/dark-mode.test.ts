...
describe('dark mode', () => {
  ...
  it('uses exact-size transparent dark logo variants', () => {
    ...
    expect(pngSize(darkWordmark)).toMatchObject({
      ...pngSize(lightWordmark),
      colourType: 6,
    });
    expect(pngSize(darkLogo)).toMatchObject({
      ...pngSize(lightLogo),
      colourType: 6,
    });
    expect(read('components/brand-wordmark.tsx')).toContain(
      '/brand/dp-wordmark-dark.png',
    );
    expect(read('components/brand-mark.tsx')).toContain(
      '/brand/dp-logo-dark.png',
    );
  });
