...
describe('support workflow and admin search pass',()=>{
  it('support submission has a single request state, checks response.ok before success, and preserves failed content',()=>{
    ...
    expect(form).toContain("type RequestState='idle'|'submitting'|'success'|'error'");
    expect(form.indexOf('if(!res.ok||!json?.ticket)')).toBeLessThan(form.indexOf("setRequestState('success')"));
  ...
  it('email search endpoint is admin-only and the shared input is used in all admin sections',()=>{
    expect(read('app/api/admin/users/search/route.ts')).toContain('requireAdmin');
    const admin=read('app/admin/admin-console.tsx');
    expect((admin.match(/EmailSearchInput/g)||[]).length).toBeGreaterThanOrEqual(4);
