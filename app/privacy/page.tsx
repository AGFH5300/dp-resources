...
export const metadata: Metadata = publicPageMetadata({
  ...
  description:
    'How DP Resources handles account details, resource usage analytics, support messages, and privacy-related requests.',
  path: '/privacy',
});
    ...
    <main className="min-h-screen bg-[#f6f1e8] text-[#10243f]">
      ...
      <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8">
        <section className="rounded-3xl border border-[#e5dccd] bg-white p-6 shadow-sm sm:p-8">
          ...
          <h1 className="mt-3 text-3xl font-semibold tracking-tight text-[#10243f] sm:text-4xl">
            Clear, limited platform records.
          </h1>
          <p className="mt-4 max-w-3xl text-base leading-7 text-slate-600">
            DP Resources is a free study resource platform. Users need an
            account so the platform can save preferences, manage support
            requests, and keep the library reliable. The platform keeps only the
            records needed to operate the library, support users, and improve
            resources.
          </p>
        </section>
        ...
        <section className="mt-6 grid gap-4 md:grid-cols-2">
          {sections.map((section) => (
            <article
              key={section.title}
              className="rounded-2xl border border-[#e5dccd] bg-white p-5 shadow-sm"
            >
              <h2 className="text-lg font-semibold text-[#10243f]">
                {section.title}
              </h2>
