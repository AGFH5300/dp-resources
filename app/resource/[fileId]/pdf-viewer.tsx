...
type LoadingPhase = 'network' | 'page' | 'ready';
...
export function PdfViewer({ url, fileId, name }: { url: string; fileId: string; name: string }) {
  ...
  const [phase, setPhase] = useState<LoadingPhase>('network');
    ...
    (async () => {
      try {
        ...
        loadingTask.onProgress = ({ loaded, total }) => {
          ...
          const safeTotal = Number.isFinite(total) && Number(total) > 0 ? Number(total) : null;
          ...
          const canEstimate = safeTotal !== null && smoothedBytesPerSecond > 0 && now - startedAt >= 500 && safeLoaded < safeTotal;
          setDownloadedBytes(safeLoaded);
          setDownloadTotal(safeTotal);
          setDownloadEtaSeconds(canEstimate ? Math.max(1, Math.ceil((safeTotal - safeLoaded) / smoothedBytesPerSecond)) : safeTotal !== null && safeLoaded >= safeTotal ? 0 : null);
        };
