...
export function ResourceUsageTracker({ fileId }: { fileId: string }) {
  ...
  useEffect(() => {
    ...
    const isActive = () =>
      ...
      document.hasFocus() &&
      Date.now() - lastActivity.current < 120_000;
    ...
    function heartbeat() {
      ...
      void fetch(`/api/resource-usage/${sessionRef.current}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ pageVisible: isActive() }),
      }).catch(() => undefined);
    }
    function end() {
      stopped = true;
    ...
    ['mousemove', 'keydown', 'pointerdown', 'scroll', 'touchstart'].forEach(
      (e) => window.addEventListener(e, active, { passive: true }),
    );
    ...
    return () => {
      ...
      ['mousemove', 'keydown', 'pointerdown', 'scroll', 'touchstart'].forEach(
        (e) => window.removeEventListener(e, active),
      );
      end();
    };
  }, [fileId]);
