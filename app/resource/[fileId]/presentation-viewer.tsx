...
function PresentationLoadingOverlay({ phase, downloadedBytes, downloadTotal, downloadEtaSeconds, renderedSlides }: { phase: LoadingPhase; downloadedBytes: number; downloadTotal: number | null; downloadEtaSeconds: number | null; renderedSlides: number }) {
  ...
  return <div className="dp-loading-overlay absolute inset-0 z-10 grid place-items-center px-6 backdrop-blur-sm" aria-label="Loading presentation preview" role="status" aria-live="polite"><div className="dp-loading-card w-full max-w-md rounded-2xl border p-5 shadow-lg"><div className="flex items-center justify-between gap-4"><div><p className="text-sm font-semibold text-[color:var(--dp-navy)]">Preparing presentation preview</p><p className="mt-1 text-xs text-slate-500">{status}</p></div><span className="text-sm font-semibold text-[color:var(--dp-navy)]">{badge}</span></div><div className="dp-loading-track mt-4 h-2 overflow-hidden rounded-full" role={downloadPercent === null ? undefined : 'progressbar'} aria-label={downloadPercent === null ? undefined : 'Presentation download progress'} aria-valuemin={downloadPercent === null ? undefined : 0} aria-valuemax={downloadPercent === null ? undefined : 100} aria-valuenow={downloadPercent ?? undefined}>{downloadPercent !== null ? <div className="dp-loading-bar h-full rounded-full transition-[width] duration-150" style={{ width: `${downloadPercent}%` }} /> : <div className="dp-loading-bar h-full w-2/5 animate-pulse rounded-full" />}</div><div className="mt-2 flex flex-wrap items-center justify-between gap-x-4 gap-y-1 text-xs text-slate-600"><span>{downloadTotal ? `Total size: ${formatBytes(downloadTotal)}` : 'Total size: calculating…'}</span><span>{etaLabel}</span></div><p className="mt-3 text-xs leading-5 text-slate-500">{phase === 'download' ? 'Download progress reflects the actual presentation bytes received.' : 'Slide rendering does not expose a percentage, so the live slide count is shown instead.'}</p></div></div>;
}
