...
async function countRows(table: 'profiles' | 'activity_logs', build: (q: any) => any) {
...
export default async function Admin({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const sb = createSupabaseAdminClient();
  const { data: profiles = [], error: profilesError } = await sb.from('profiles').select('*').order('created_at', { ascending: false });
  if (profilesError) throw new Error(profilesError.message);
  let activityQuery = sb.from('activity_logs').select('*').order('created_at', { ascending: false }).limit(100);
