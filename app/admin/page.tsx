...
export default async function Admin({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  let usersQ=sb.from('dp_resource_memberships').select('*',{count:'exact'}).order('created_at',{ascending:false}); if(sp.userEmail) usersQ=usersQ.ilike('email',`%${sp.userEmail}%`); if(sp.role) usersQ=usersQ.eq('role',sp.role); const {data:membershipRows=[],count:userCount=0,error:membershipsError}=await usersQ.range((userPage-1)*userSize,userPage*userSize-1); if(membershipsError) throw new Error(membershipsError.message); const latestActivity=new Map<string,string>(); if((membershipRows||[]).length){ const {data:recentUserLogs=[]}=await sb.from('dp_resource_activity_logs').select('user_id,created_at').in('user_id',(membershipRows||[]).map(u=>u.id)).order('created_at',{ascending:false}).limit(500); (recentUserLogs||[]).forEach(l=>{ if(!latestActivity.has(l.user_id)) latestActivity.set(l.user_id,l.created_at); }); } const memberships=(membershipRows||[]).map(u=>({...u,latest_activity_at:latestActivity.get(u.id)||null}));
