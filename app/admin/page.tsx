...
export default async function Admin({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  let memberships:any[]=[]; let logs:any[]=[]; let reports:any[]=[]; let tickets:any[]=[]; let usage:any[]=[]; let diagnostics:any[]=[]; let userCount=0; let activityCount=0; let reportCount=0; let ticketCount=0; let indexStatus:any=null;
  ...
  if (section === 'index') {
    ...
    const range=sp.range||'30d'; const {data=[],error}=await sb.rpc('dp_admin_resource_usage_leaderboard',{p_range:range,p_limit:100}); if(error) throw new Error(error.message); usage=data||[];
  } else if (section === 'diagnostics') {
    const {data=[]}=await sb.from('dp_server_error_events').select('occurred_at,level,area,message,context').order('occurred_at',{ascending:false}).limit(50); diagnostics=data||[];
