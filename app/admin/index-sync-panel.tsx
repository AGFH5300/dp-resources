...
export function IndexSyncPanel({ initial }: { initial: Payload }) {
  ...
  const queuedPages =
    data.state?.folder_queue?.filter((item) => item.pageToken).length || 0;
  const status = data.state?.status || 'idle';
  const lockActive =
    status === 'indexing' &&
    data.state?.lock_expires_at &&
    new Date(data.state.lock_expires_at).getTime() > Date.now();
  ...
  const message = useMemo(() => {
    ...
    if (data.error || status === 'failed')
      return `Paused — ${remaining.toLocaleString()} folder pages remaining`;
    if (status === 'complete') return 'Index complete';
    if (running && data.state?.indexed_resources)
      return `Indexing ${data.state.indexed_resources.toLocaleString()} current-run items…`;
    if (remaining > 0)
      return `${remaining.toLocaleString()} folder pages remaining`;
  ...
  return (
    <section className="mt-8 border border-slate-200 bg-white p-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          <button
            type="button"
            disabled={inFlight || Boolean(lockActive)}
            onClick={runNextChunk}
            className="rounded-md bg-slate-950 px-3 py-2 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-60"
          >
            {lockActive
              ? 'Indexing…'
      ...
      <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-3 lg:grid-cols-6">
        <div>
          <dt className="text-xs uppercase text-slate-500">Files indexed</dt>
          <dd className="font-medium text-slate-900">
            {filesIndexed.toLocaleString()}
          </dd>
        </div>
        <div>
          <dt className="text-xs uppercase text-slate-500">Folders indexed</dt>
          <dd className="font-medium text-slate-900">
            {foldersIndexed.toLocaleString()}
          </dd>
        </div>
        <div>
          ...
          <dd className="font-medium text-slate-900">
            {(data.totalIndexed || 0).toLocaleString()}
          </dd>
        </div>
        <div>
          <dt className="text-xs uppercase text-slate-500">
          ...
          <dd className="font-medium text-slate-900">
            {(data.state?.processed_folders || 0).toLocaleString()}
          </dd>
        </div>
        <div>
          <dt className="text-xs uppercase text-slate-500">
        ...
        <div>
          <dt className="text-xs uppercase text-slate-500">
            Current-run items discovered
          </dt>
          <dd className="font-medium text-slate-900">
            {(data.state?.indexed_resources || 0).toLocaleString()}
          </dd>
        </div>
      </dl>
      {remaining > 0 && (
        <p className="mt-2 text-xs text-slate-500">
          Queue can grow while nested folders and paginated folder pages are
          discovered. Total indexed items may include retained rows from earlier
          partial work and is not a file-only total. Continuation pages queued:{' '}
          {queuedPages.toLocaleString()}.
        </p>
      )}
      {data.error && (
        <p className="mt-2 text-xs text-amber-700">{data.error}</p>
      )}
      <p className="mt-3 text-xs text-slate-500">
        Indexing runs in safe resumable batches and cleans stale rows only after
        a successful full sync.
      </p>
    </section>
  );
}
