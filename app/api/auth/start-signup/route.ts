...
export async function POST(request: Request) {
  ...
  const limited = await rateLimit(
    privacySafeRequestKey(request, 'signup-start'),
    8,
    10 * 60 * 1000,
    'signup-start',
  );
  ...
  try {
    ...
    return jsonResponse(
      { ok: false, message: 'Could not read signup details.', field: 'form' },
      400,
    );
  }
    ...
    return jsonResponse(
      {
        ...
        field: 'username',
        debug: { path: 'username_identity_failed' },
      },
      400,
    );
  }
  ...
  if (!fullNamePolicy.ok) {
    ...
    return jsonResponse(
      {
        ok: false,
        message: 'Enter an appropriate name.',
        field: 'fullName',
        debug: { reason: fullNamePolicy.reason },
      },
      400,
    );
  }
  ...
  if (!isValidEmail(email)) {
    return jsonResponse(
      { ok: false, message: 'Enter a valid email address.', field: 'email' },
      400,
    );
  }
    ...
    return jsonResponse(
      {
        ...
        field: 'email',
        debug: { reason: emailPolicy.reason },
      },
      400,
    );
  }
    ...
    return jsonResponse(
      {
        ...
        field: 'form',
        debug: { path: 'email_domain_policy_failed' },
      },
      500,
    );
  }
    ...
    return jsonResponse(
      {
        ...
        field: 'email',
        debug: {
          path: 'email_domain_policy_blocked',
        ...
        debug: signupDebugEnabled
          ? {
              ...
              usernameError: usernameError
                ? { code: usernameError.code, message: usernameError.message }
                : null,
              emailError: emailError
                ? { code: emailError.code, message: emailError.message }
                : null,
            }
          : undefined,
      },
