...
export async function GET(request: Request) {
  const limited = await rateLimit(
    privacySafeRequestKey(request, 'signup-availability'),
    60,
    10 * 60 * 1000,
    'signup-availability',
  );
  ...
  if (!rawType && !rawValue) {
    return jsonResponse(
      'error',
      false,
      'Availability check requires both type and value query params.',
      {
        status: 200,
  ...
  if (!type) {
    return jsonResponse(
      'error',
      false,
      'Invalid check type. Use type=username or type=email.',
      {
        status: 400,
  ...
  if (type === 'email') {
    ...
    if (!emailPolicy.ok) {
      ...
      return jsonResponse('invalid', false, 'Use a different email address.', {
        debug: {
          ...requestMeta,
          validationPath: 'email_policy_failed',
  ...
  if (type === 'username') {
      ...
      return jsonResponse(
        ...
        'Could not validate username right now.',
        {
          ...
          debug: {
            ...requestMeta,
            validationPath:
              'rpc_dp_resource_username_availability_status_failed',
    ...
    const usernameStatus =
      data === 'available' || data === 'unavailable' || data === 'invalid'
        ? data
        : 'error';
    ...
    return jsonResponse(
          ...
          : 'That username is already taken.',
      {
        debug: {
          ...requestMeta,
          validationPath:
            'rpc_dp_resource_username_availability_status_succeeded',
  ...
  try {
    ...
    return jsonResponse('error', false, 'Could not validate email right now.', {
      ...
      debug: {
        ...requestMeta,
        validationPath: 'rpc_dp_resource_email_domain_policy_failed',
        error: policyError instanceof Error ? policyError.message : 'unknown',
      },
    });
  }
  ...
  if (domainPolicy.allowed !== true) {
    return jsonResponse('invalid', false, DISPOSABLE_EMAIL_MESSAGE, {
      ...
      debug: {
        ...requestMeta,
        validationPath: 'email_domain_policy_blocked',
