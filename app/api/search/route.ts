...
function indexAvailability(state:any,count:number){const available=Boolean(state?.completed_at)&&count>0;return{available,updating:available&&state?.status!=='complete'};}
...
export async function GET(req:Request){
  ...
  const {available,updating}=indexAvailability(state,count||0); const indexState=updating?'updating':available?'ready':'preparing';
  if(needle.length<2)return Response.json({folders:[],files:[],indexState});
  if(!available)return Response.json({folders:[],files:[],indexState:'preparing'});
  const key=needle.toLowerCase(); const hit=cache.get(key); if(hit&&Date.now()-hit.t<15_000)return Response.json({...hit.payload,indexState},{headers:process.env.NODE_ENV==='development'?{'Server-Timing':`search;dur=${(performance.now()-start).toFixed(1)}`}:undefined});
  ...
  const payload={folders:rows.filter((r:any)=>r.is_folder),files:rows.filter((r:any)=>!r.is_folder)};
  cache.set(key,{t:Date.now(),payload});
  return Response.json({...payload,indexState},{headers:process.env.NODE_ENV==='development'?{'Server-Timing':`search;dur=${(performance.now()-start).toFixed(1)}`}:undefined});
}
