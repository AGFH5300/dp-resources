...
export async function GET(
  ...
  const preview = await getPdfPreviewDocumentByIdentity(
    session.previewId,
  ...
  return Response.json(
    {
      ...
      manifestUrl: viewable
        ? `/api/resource/${encodeURIComponent(fileId)}/pdf-preview/manifest`
        : null,
      searchReady: Boolean(
        preview.text_ready_at && preview.search_geometry_ready_at,
      ),
      message:
        preview.status === 'failed'
          ? 'PDF preview preparation failed'
          : undefined,
    },
    {
