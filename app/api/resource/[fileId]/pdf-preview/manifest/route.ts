import {
  getPdfPreviewManifestByIdentity,
...
export async function GET(
  ...
  const manifest = await getPdfPreviewManifestByIdentity(
    session.previewId,
    session.previewVersionKey,
  ).catch(() => null);
  if (!manifest || !isPdfPreviewViewable(manifest.document)) {
    return Response.json(
      { status: manifest?.document.status || 'queued' },
      {
        status: 202,
  ...
  return Response.json(
    {
      ...
      searchReady: Boolean(
        manifest.document.text_ready_at &&
          manifest.document.search_geometry_ready_at,
      ),
