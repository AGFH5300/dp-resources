...
export async function PATCH(
  ...
  try {
    ...
    ['status', 'assigned_to', 'internal_notes', 'admin_notes'].forEach((k) => {
      if (k in body)
        update[k === 'admin_notes' ? 'internal_notes' : k] = body[k] || null;
    });
    ...
    return Response.json(
      {
        error:
          e instanceof Error ? e.message : 'Could not update support ticket',
      },
      { status: 500 },
    );
  }
