...
export async function POST(request: Request){const forbidden=sameOriginOrForbidden(request); if(forbidden)return forbidden;await requireAdmin(); if(!isDriveConfigured())return Response.json({error:'Drive not configured'},{status:503}); try{return Response.json(await runIndexSyncChunk());}catch(e){return Response.json({error:e instanceof Error?e.message:'Index sync failed'},{status:500});}}
