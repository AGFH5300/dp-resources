...
export function UnsuspensionWatcher({
  initialUserId,
}: UnsuspensionWatcherProps) {
  const navigatedRef = useRef(false);
  const userId = useMemo(
    () => nonEmptyString(initialUserId) ?? null,
    [initialUserId],
  );
  ...
  useEffect(() => {
    const resolvedUserId =
      userId ??
      nonEmptyString(
        window.sessionStorage.getItem(SUSPENDED_USER_ID_STORAGE_KEY),
      );
    ...
    async function checkStatus() {
      ...
      try {
        ...
        if (!active || navigatedRef.current || status?.authenticated !== true)
          return;
