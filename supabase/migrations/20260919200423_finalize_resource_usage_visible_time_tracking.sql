-- Final production state for visible resource-time tracking.
--
-- The browser sends heartbeats every 10 seconds while the resource page remains
-- visible and pauses them while hidden. Allow up to five minutes of bounded
-- recovery when a heartbeat request is delayed, matching the API's 300-second
-- safety cap. The existing row lock and wall-clock bound prevent double credit.
