...
insert into public.dp_resource_email_domain_rules_compact (
  domain, action, reason, source, created_by, created_at, updated_at
)
select
  old_rule.domain, old_rule.action, old_rule.reason, old_rule.source,
  old_rule.created_by, old_rule.created_at, old_rule.updated_at
...
begin
  select count(*) into old_count from public.dp_resource_email_domain_rules;
  select
  ...
  if old_count <> partition_count then
    raise exception 'Disposable-email compaction count mismatch: old %, compact %', old_count, partition_count;
  end if;

  if exists (
    select 1
    from public.dp_resource_disposable_email_domains bulk
    join public.dp_resource_email_domain_rules_compact compact_rule using (domain)
  ) then
...
revoke all on public.dp_resource_disposable_email_domains from public, anon, authenticated;
revoke all on public.dp_resource_email_domain_rules_compact from public, anon, authenticated;
grant select, insert, update, delete on public.dp_resource_disposable_email_domains to service_role;
grant select, insert, update, delete on public.dp_resource_email_domain_rules_compact to service_role;
...
begin
  ...
  with recursive candidates(domain, depth) as (
    select v_domain, 0
    union all
    select substring(candidate.domain from position('.' in candidate.domain) + 1), candidate.depth + 1
    from candidates candidate
    where position('.' in candidate.domain) > 0
  ), matched as (
    select candidate.domain as matched_domain, candidate.depth, rule.action, 0 as source_priority
    from candidates candidate
    join public.dp_resource_email_domain_rules rule on rule.domain = candidate.domain
    union all
    select candidate.domain as matched_domain, candidate.depth, 'block'::text as action, 1 as source_priority
    from candidates candidate
    join public.dp_resource_disposable_email_domains bulk on bulk.domain = candidate.domain
  )
  select matched_domain, action into v_match
  from matched
  ...
  return jsonb_build_object('allowed', true, 'domain', v_domain, 'matched_domain', null, 'reason', null);
end;
$$;
...
alter table public.dp_resource_email_domain_rules_compact rename to dp_resource_email_domain_rules;

alter table public.dp_resource_email_domain_rules
  rename constraint dp_resource_email_domain_rules_compact_pkey to dp_resource_email_domain_rules_pkey;
alter table public.dp_resource_email_domain_rules
  rename constraint dp_resource_email_domain_rules_compact_action_valid to dp_resource_email_domain_rules_action_valid;
alter table public.dp_resource_email_domain_rules
  rename constraint dp_resource_email_domain_rules_compact_normalized to dp_resource_email_domain_rules_domain_normalized;
alter table public.dp_resource_email_domain_rules
  rename constraint dp_resource_email_domain_rules_compact_created_by_fkey to dp_resource_email_domain_rules_created_by_fkey;

revoke all on public.dp_resource_email_domain_rules from public, anon, authenticated;
grant select, insert, update, delete on public.dp_resource_email_domain_rules to service_role;
