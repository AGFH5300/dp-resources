...
async function repairMissingResourceMembership(user: { id: string; email: string }) {
  ...
  const { error } = await sb
    ...
    .upsert(pendingMembershipInsert(user), { onConflict: 'id', ignoreDuplicates: true });
  if (error) throw new Error(`Unable to repair missing membership: ${error.message}`);
  ...
  if (readError) throw new Error(`Unable to read repaired membership: ${readError.message}`);
  return membership;
}
...
export async function getSessionResourceMembership() {
  ...
  const { data: membership, error } = await supabase.from('dp_resource_memberships').select('*').eq('id', user.id).maybeSingle<ResourceMembership>();
  if (error) throw new Error(`Unable to read membership: ${error.message}`);
  return { user, membership: membership || await repairMissingResourceMembership({ id: user.id, email: user.email }) };
}
