...
type ActivityInput = { userId: string; userEmail: string; fileId?: string | null; fileName: string; action: ActivityLog['action'] };
...
export async function recordActivity(input: ActivityInput) {
  ...
  await createSupabaseAdminClient().from('dp_resource_activity_logs').insert({
...
export async function recordFileOpenedOnce(req: Request, input: Omit<ActivityInput, 'action'>) {
