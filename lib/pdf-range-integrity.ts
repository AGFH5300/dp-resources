...
export function expectedPdfContentRange(
  decision: RangeDecision,
  totalSize: number,
) {
  return decision.kind === 'range'
    ? `bytes ${decision.start}-${decision.end}/${totalSize}`
    : null;
}

export function validatePdfRangeUpstream(
  decision: RangeDecision,
  totalSize: number,
  ...
  if (
    ...
    expectedPdfContentRange(decision, totalSize)
  )
