...
const payload = Buffer.from(
  `dp-resources-pdf-preview-preflight:${randomUUID()}`,
  'utf8',
);
let uploaded = false;
...
try {
  ...
  if (!response.ok)
    throw new Error(
      `R2 preflight object was not readable (status ${response.status})`,
    );
  const downloaded = Buffer.from(await response.arrayBuffer());
  if (!downloaded.equals(payload))
    throw new Error('R2 preflight read did not match the uploaded bytes');

  console.log(
    JSON.stringify({ event: 'pdf_preview_r2_preflight_ready', bucket }),
  );
} finally {
