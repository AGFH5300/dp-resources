...
function isLikelyTextbook(file) {
  ...
  const excluded = /(^|\b)(question paper|markscheme|mark scheme|specimen paper|past paper|student notes|worksheet|mind[- ]?map)(\b|$)/i.test(name)
    || /^\d{4}\s+(may|november)\b/i.test(name)
    || /topic\s+\w*\d+.*sample/i.test(name);
  if (excluded) return false;
  const strongTextbookSignal = /(textbook|coursebook|course companion|study guide|revision guide|worked solutions|teacher'?s? guide|pearson|cambridge|oxford|hodder|\bibid\b|\bexpress\b|\bedition\b)/i.test(name);
...
async function storageUsage(provider) {
  const { data, error } = await supabase.rpc('dp_pdf_preview_storage_usage', { p_storage_provider: provider });
  if (error) throw new Error(`Unable to read ${provider} preview storage usage: ${error.message}`);
  return Number(data || 0);
}

async function documentUsage(documentId) {
  const { data, error } = await supabase.rpc('dp_pdf_preview_document_storage_usage', { p_document_id: documentId });
...
async function targetDocument(file) {
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error) throw new Error(`Unable to read preview state for ${file.name}: ${error.message}`);
  return data || null;
}
...
async function writeSummary(results, skipped, stoppedForStorage, finalStorageBytes) {
...
async function main() {
  ...
  console.log(JSON.stringify({
    ...
    minimumBytes,
    maxBooks,
    selected: selected.map((file) => ({ fileId: file.drive_file_id, name: file.name, sizeBytes: file.size_bytes })),
  ...
  for (const file of selected) {
    ...
    results.push(result);
    console.log(JSON.stringify({ event: 'pdf_preview_batch_result', ...result }));
  }
