...
const minSizeArgument = process.argv.find((value) =>
  value.startsWith('--min-size-mb='),
);
const minSizeMb = Number(
  minSizeArgument?.split('=')[1] ||
    process.env.PDF_PREVIEW_QUEUE_MIN_SIZE_MB ||
    0,
);
...
if (driveFileArgument && !driveFileId)
  throw new Error('Drive file ID must not be empty');
...
async function queueFile(file) {
  ...
  if (!document)
    throw new Error(`Queueing did not return a document for ${file.name}`);
  ...
  if (
    ...
    !document.locked_by &&
    (document.storage_provider !== requestedProvider ||
      document.storage_bucket !== requestedBucket)
  ) {
    ...
    if (updateError)
      throw new Error(
        `Unable to select ${requestedProvider} storage for ${file.name}: ${updateError.message}`,
      );
...
async function main() {
  ...
  while (true) {
    ...
    if (error)
      throw new Error(`Unable to read PDF resources: ${error.message}`);
    if (!data?.length) break;

    for (const file of data) {
      if (
        !Number.isSafeInteger(Number(file.size_bytes)) ||
        Number(file.size_bytes) <= 0
      )
        continue;
      const result = await queueFile({
        ...file,
