...
const UPLOAD_CONCURRENCY = Number(
  process.env.PDF_PREVIEW_UPLOAD_CONCURRENCY || 6,
);
...
const workerId =
  process.env.PDF_PREVIEW_WORKER_ID || `pdf-worker-${randomUUID()}`;
...
if (!Number.isFinite(RENDER_DPI) || RENDER_DPI < 96 || RENDER_DPI > 240)
  throw new Error('PDF_PREVIEW_DPI must be between 96 and 240');
if (!Number.isFinite(JPEG_QUALITY) || JPEG_QUALITY < 50 || JPEG_QUALITY > 95)
  throw new Error('PDF_PREVIEW_JPEG_QUALITY must be between 50 and 95');
if (!Number.isSafeInteger(BATCH_SIZE) || BATCH_SIZE < 1 || BATCH_SIZE > 100)
  throw new Error('PDF_PREVIEW_BATCH_SIZE must be between 1 and 100');
if (
  !Number.isSafeInteger(UPLOAD_CONCURRENCY) ||
  UPLOAD_CONCURRENCY < 1 ||
  UPLOAD_CONCURRENCY > 12
)
  throw new Error('PDF_PREVIEW_UPLOAD_CONCURRENCY must be between 1 and 12');
if (
  !Number.isSafeInteger(UPLOAD_ATTEMPTS) ||
  UPLOAD_ATTEMPTS < 1 ||
  UPLOAD_ATTEMPTS > 8
)
  throw new Error('PDF_PREVIEW_UPLOAD_ATTEMPTS must be between 1 and 8');
...
function errorDetails(error) {
  ...
  if (error.error && error.error !== error.message)
    details.push(String(error.error));
  return details.join(' ');
}
...
async function withUploadRetry(pageNumber, operation) {
  ...
  throw new Error(
    `Unable to upload PDF page ${pageNumber}: ${errorDetails(lastError)}`,
  );
}
...
async function claimJob() {
  const { data, error } = await supabase.rpc('dp_claim_pdf_preview_job', {
    p_worker_id: workerId,
  });
  if (error)
    throw new Error(`Unable to claim PDF preview job: ${error.message}`);
...
async function downloadSource(job, destination) {
  ...
  if (Number(details.size) !== Number(job.source_size_bytes)) {
    throw new Error(
      `Drive PDF size changed during preparation (${details.size} != ${job.source_size_bytes})`,
    );
  }
}
...
function parsePdfInfo(output) {
  ...
  if (dimensions.size !== pageCount)
    throw new Error(
      `pdfinfo returned dimensions for ${dimensions.size} of ${pageCount} pages`,
    );
...
async function upsertPageDimensions(job, pageCount, dimensions) {
  ...
  for (let pageNumber = 1; pageNumber <= pageCount; pageNumber += 1) {
    ...
    rows.push({
      ...
      pixel_width: Math.max(
        1,
        Math.round((page.widthPoints * RENDER_DPI) / 72),
      ),
      pixel_height: Math.max(
        1,
        Math.round((page.heightPoints * RENDER_DPI) / 72),
      ),
  ...
  for (let offset = 0; offset < rows.length; offset += 500) {
    ...
    if (error)
      throw new Error(`Unable to save PDF page dimensions: ${error.message}`);
  }
  const { error } = await supabase
...
function parseBboxSearchIndex(output, pageCount) {
  ...
  for (const pageMatch of output.matchAll(pagePattern)) {
    ...
    if (!(width > 0) || !(height > 0))
      throw new Error('pdftotext returned invalid page geometry');
    const words = [];
    const lines = [];
    let lineNumber = 0;
  ...
  if (pages.length !== pageCount)
    throw new Error(
      `pdftotext returned geometry for ${pages.length} of ${pageCount} pages`,
    );
  return pages;
}
...
async function extractPageSearchIndex(sourcePath, outputPath, pageCount) {
  await execFile(
    'pdftotext',
    ['-bbox-layout', '-enc', 'UTF-8', sourcePath, outputPath],
    {
...
async function storePageText(job, pages) {
  ...
  if (error)
    throw new Error(`Unable to save PDF search text: ${error.message}`);
  if (Number(data) !== payload.length) {
    throw new Error(
      `Saved search text for ${Number(data) || 0} of ${payload.length} PDF pages`,
    );
  }
}
...
async function uploadSearchGeometry(job, pages) {
  ...
  if (error)
    throw new Error(
      `Unable to mark PDF search geometry ready: ${error.message}`,
    );
}
...
async function existingReadyPages(jobId) {
  ...
  while (true) {
    const { data, error } = await supabase
      ...
      .range(from, from + 999);
    if (error)
      throw new Error(`Unable to read prepared PDF pages: ${error.message}`);
...
async function renderRange(sourcePath, outputDir, start, end) {
  ...
  const args = [
    ...
    String(RENDER_DPI),
    '-jpegopt',
    `quality=${JPEG_QUALITY},progressive=y,optimize=y`,
    sourcePath,
    prefix,
  ];
  ...
  const files = (await readdir(outputDir))
    .filter(
      (file) => file.startsWith(basename(prefix)) && file.endsWith('.jpg'),
    )
    .sort();
  if (files.length !== end - start + 1)
    throw new Error(
      `Rendered ${files.length} of ${end - start + 1} requested pages`,
    );
...
async function uploadRenderedBatch(job, dimensions, rendered) {
  ...
  if (error)
    throw new Error(`Unable to record PDF page batch: ${error.message}`);
...
async function updateProgress(job, pageCount, pagesReady) {
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      first_page_ready_at:
        pagesReady > 0 ? job.first_page_ready_at || now : null,
      completed_at: complete ? now : null,
      locked_by: complete ? null : workerId,
      lock_expires_at: complete
        ? null
    ...
    .eq('id', job.id);
  if (error)
    throw new Error(`Unable to update PDF preview progress: ${error.message}`);
}
...
async function renderAndRecord(
  ...
  for (const item of rendered) {
    if (!pending.some((pendingItem) => pendingItem.path === item.path))
      await rm(item.path, { force: true });
  }
  await updateProgress(job, pageCount, readyPages.size);
  console.log(
    JSON.stringify({
      event: 'pdf_preview_progress',
      fileId: job.drive_file_id,
      pagesReady: readyPages.size,
      pageCount,
    }),
  );
}
...
async function processJob(job) {
  ...
  try {
    console.log(
      JSON.stringify({
        event: 'pdf_preview_started',
        workerId,
        fileId: job.drive_file_id,
        version: job.version_key,
      }),
    );
    ...
    console.log(
      JSON.stringify({
        event: 'pdf_preview_resume_state',
        fileId: job.drive_file_id,
        pagesReady: readyPages.size,
        pageCount,
        searchReady: Boolean(job.text_ready_at && job.search_geometry_ready_at),
      }),
    );

    if (!readyPages.has(1)) {
      await renderAndRecord(
        job,
        sourcePath,
        outputDir,
        dimensions,
    ...
    if (!job.text_ready_at || !job.search_geometry_ready_at) {
      try {
        const pages = await extractPageSearchIndex(
          sourcePath,
          textPath,
          pageCount,
        );
    ...
    for (let start = 2; start <= pageCount; start += BATCH_SIZE) {
      ...
      if (
        [...Array(end - start + 1)].every((_, index) =>
          readyPages.has(start + index),
        )
      )
        continue;
      await renderAndRecord(
    ...
    if (readyPages.size >= pageCount)
      await updateProgress(job, pageCount, readyPages.size);
    console.log(
      JSON.stringify({
        event: 'pdf_preview_ready',
        fileId: job.drive_file_id,
        pageCount,
        searchReady: Boolean(job.text_ready_at && job.search_geometry_ready_at),
      }),
