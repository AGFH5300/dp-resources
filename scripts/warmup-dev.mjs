...
function toAppRoute(filePath) {
  ...
  if (rel.startsWith(`api${path.sep}`) && /(?:^|\/)route\.(ts|js)$/.test(rel)) {
    ...
    segs.pop(); // route.ts
    return (
      '/api/' + segs.slice(1).map(normalizeSegment).filter(Boolean).join('/')
    );
  }
  // page.* files -> pages
...
function toPagesRoute(filePath) {
  ...
  if (rel.startsWith('api/')) {
    ...
    return (
      '/' + withNoExt.split('/').map(normalizeSegment).filter(Boolean).join('/')
    );
  }
  // ignore _app, _document, _error, 404
  if (/^(_app|_document|_error|404)\.(js|jsx|tsx)$/.test(path.basename(rel)))
    return null;
...
(async () => {
  ...
  console.log(
    `[warmup] Done in ${took}s — ${ok}/${results.length} successful.`,
  );

  const failures = results.filter((r) => !r.ok);
  if (failures.length) {
    console.log(
      `[warmup] Non-2xx responses (often expected for protected/admin/API):`,
    );
    failures.forEach((f) =>
