...
export function GlobalSearch() {
  ...
  const clearState=()=>{setQ('');setFolders([]);setFiles([]);setError('');setLoading(false);setSlow(false);setActive(0);};
  ...
  return <div className="fixed inset-0 z-50 bg-[color:var(--dp-navy)]/25 p-3 backdrop-blur-[2px]" role="dialog" aria-modal="true" onMouseDown={(e)=>{if(e.target===e.currentTarget)close();}}>
    <div className="mx-auto mt-18 w-full max-w-[640px] overflow-hidden rounded-lg border border-stone-200/80 bg-[color:var(--dp-warm-surface)] shadow-[0_18px_55px_rgb(30_41_59/0.18)]">
      <div className="flex h-14 items-center gap-3 border-b border-stone-200/80 px-4"><Search className="size-5 text-slate-500" /><input ref={input} value={q} onChange={e=>setQ(e.target.value)} onKeyDown={(e)=>{if(e.key==='ArrowDown'){e.preventDefault();setActive(a=>Math.min(a+1,Math.max(flat.length-1,0)));}if(e.key==='ArrowUp'){e.preventDefault();setActive(a=>Math.max(a-1,0));}if(e.key==='Enter'&&choose){const href=resourceUrl({drive_file_id:choose.drive_file_id,is_folder:choose.is_folder}); resetSearch(); location.href=href;}}} placeholder="Search files, folders, and paths" className="flex-1 text-base" style={{background:'transparent',backgroundColor:'transparent',border:'none',borderColor:'transparent',outline:'none',boxShadow:'none'}} /><button aria-label="Close search" onClick={close} className="rounded-md p-1 text-slate-500 hover:bg-slate-100"><X className="size-5" /></button></div>
      <div className="max-h-[52vh] overflow-y-auto p-2">
        {q.length < 2 ? <div className="px-3 py-4"><p className="text-sm font-medium text-[color:var(--dp-navy)]">Search files, folders, and paths</p><p className="mt-1 text-xs text-[color:var(--dp-ink)]/60">Type at least two characters.</p>{indexState==='preparing'&&<p className="mt-2 text-xs text-[color:var(--dp-ink)]/50">Indexing is active; results may be incomplete.</p>}</div> : loading ? <p className="p-8 text-center text-sm text-[color:var(--dp-ink)]/65">{slow ? 'Still searching…' : 'Searching your library…'}</p> : error ? <div className="p-8 text-center"><p className="text-sm text-red-700">{error}</p><button onClick={()=>setQ(q+' ')} className="mt-2 text-sm font-medium text-[color:var(--dp-navy)]">Retry</button></div> : !flat.length ? <div className="p-8 text-center text-sm text-[color:var(--dp-ink)]/65"><p>{indexState==='empty'?'Your administrator needs to sync the library before global search is available.':'No matching resources.'}</p>{indexState==='empty'&&<Link onClick={resetSearch} href="/admin" className="mt-3 inline-flex rounded-md border border-slate-200 bg-white px-3 py-2 font-medium text-[color:var(--dp-navy)]">Administrator sync required</Link>}</div> : <>
          ...
          {folders.length ? <section className="mb-3"><h3 className="px-2 py-1 text-xs font-semibold uppercase tracking-wide text-slate-500">Folders</h3>{folders.map((r)=>resultRow(r,q,flat.indexOf(r)===active,resetSearch))}</section> : null}
          {files.length ? <section className="mb-3"><h3 className="px-2 py-1 text-xs font-semibold uppercase tracking-wide text-slate-500">Files</h3>{files.map((r)=>resultRow(r,q,flat.indexOf(r)===active,resetSearch))}</section> : null}
