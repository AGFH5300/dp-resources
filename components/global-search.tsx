...
export function GlobalSearch() {
  ...
  return <div className="fixed inset-0 z-50 bg-[color:var(--dp-navy)]/25 p-3 backdrop-blur-[2px]" role="dialog" aria-modal="true" onMouseDown={(e)=>{if(e.target===e.currentTarget)close();}}>
    <div className="mx-auto mt-20 w-full max-w-[42rem] overflow-hidden rounded-xl border border-slate-200 bg-[color:var(--dp-warm-surface)] shadow-xl">
      <div className="flex items-center gap-3 border-b border-slate-200 px-4 py-3"><Search className="size-5 text-slate-500" /><input ref={input} value={q} onChange={e=>setQ(e.target.value)} onKeyDown={(e)=>{ if(e.key==='ArrowDown'){e.preventDefault();setActive(a=>Math.min(a+1,Math.max(flat.length-1,0)));} if(e.key==='ArrowUp'){e.preventDefault();setActive(a=>Math.max(a-1,0));} if(e.key==='Enter'&&choose){ location.href=resourceUrl({drive_file_id:choose.drive_file_id,is_folder:choose.is_folder});}}} placeholder="Search files, folders, and paths" className="flex-1 border-0 bg-transparent text-base outline-none focus-visible:shadow-none" /><button aria-label="Close search" onClick={close} className="rounded-md p-1 text-slate-500 hover:bg-slate-100"><X className="size-5" /></button></div>
      <div className="max-h-[60vh] overflow-y-auto p-3">
        {q.length < 2 ? <div className="p-8 text-center"><p className="text-sm font-medium text-[color:var(--dp-navy)]">Search files, folders, and paths</p><p className="mt-1 text-sm text-[color:var(--dp-ink)]/60">Type at least two characters.</p>{indexState==='preparing'&&<p className="mt-2 text-xs text-[color:var(--dp-ink)]/50">Search is being prepared. Results may be incomplete.</p>}</div> : loading ? <p className="p-8 text-center text-sm text-[color:var(--dp-ink)]/65">{slow ? 'Still searching…' : 'Searching your library…'}</p> : error ? <div className="p-8 text-center"><p className="text-sm text-red-700">{error}</p><button onClick={()=>setQ(q+' ')} className="mt-2 text-sm font-medium text-[color:var(--dp-navy)]">Retry</button></div> : !flat.length ? <div className="p-8 text-center text-sm text-[color:var(--dp-ink)]/65"><p>{indexState==='empty'?'Your administrator needs to sync the library before global search is available.':'No matching resources.'}</p>{indexState==='empty'&&<Link onClick={close} href="/admin" className="mt-3 inline-flex rounded-md border border-slate-200 bg-white px-3 py-2 font-medium text-[color:var(--dp-navy)]">Administrator sync required</Link>}</div> : <>
          {indexState==='preparing'&&<p className="mb-2 rounded-md bg-amber-50 p-2 text-xs text-amber-900">Search is being prepared. Results may be incomplete.</p>}
