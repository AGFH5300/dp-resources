...
function catalogConceptIndex(catalog: Catalog) {
  const index = new Map<
    string,
    { subjectName: string; groupName: string; concept: CatalogConcept }
  >();
...
export function PracticeSetBuilderV3({
  catalog,
                  ...
                  {blockPreview ? (
                    ...
                    >
                      ...
                      {blockPreview.overlapQuestionCount
                        ? ` · ${blockPreview.overlapQuestionCount.toLocaleString()} overlap another block`
                        : ''}
                      {blockPreview.shortage
                        ? ` · short by ${blockPreview.shortage.toLocaleString()}`
                        : ''}
                    </div>
                  ) : null}
                </article>
          ...
          <section className={`${styles.summaryCard} rounded-2xl p-4 shadow-lg`}>
            ...
            {totalRequested > 500 ? (
              <p className="mt-3 rounded-xl bg-white/10 px-3 py-2 text-xs text-blue-50">
                This is a large set. The fixed queue will be stored in full and shown
                in pages so you can leave and resume without loading every question at
                once.
              </p>
            ) : null}
            {preview?.overlappingQuestionCount ? (
              <p className="mt-3 rounded-xl bg-white/10 px-3 py-2 text-xs text-blue-50">
                ...
                multiple selected blocks and will appear only once.
              </p>
            ) : null}
            {previewLoading ? (
            ...
            <div className="mt-4 grid gap-2">
              ...
              <PracticeShareDialog
                ...
                disabled={!configuration || previewLoading}
