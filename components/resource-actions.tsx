...
export function ReportResourceDialog({
  ...
  const isQuestionToolbarProxy = className.includes(
    'dp-qb-toolbar-report-button',
  );
  function close() {
    if (busy) return;
    setOpen(false);
  ...
  useEffect(() => {
    if (!open || isQuestionToolbarProxy) return;
  ...
  }, [open, busy, sent, isQuestionToolbarProxy]);
